...
try:
    # Define the common key for merging (replace with the correct key, e.g., 'vehicle_id' or 'model_year')
    ...
    st.error(f"Merge failed: Missing key column {e}")
    merged_df = None
except Exception as e:
    st.error(f"An error occurred while merging datasets: {e}")
    merged_df = None
...
if section == "Introduction":
    ...
    st.write("### Individual Datasets Overview")
    for name, df in dataframes.items():
        st.subheader(name)
